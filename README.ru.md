...
**E2EE-мессенджер для web и desktop: чаты, голос, кружки, звонки.**
...
[English](README.md) · [Что умеет](#что-умеет-сейчас) · [Локальный запуск](#локальная-разработка) · [Архитектура](#архитектура) · [Модель безопасности](#модель-безопасности)
...
Chaos — E2EE-мессенджер для web и Electron. Сервер видит ciphertext, метаданные и signaling. Открытый текст сообщений, файлов и ключей остаётся на устройстве.

Это инженерный проект, не замена Signal. Своего независимого crypto-аудита нет.

Инженерные инварианты, на которых собран backend:

1. **Plaintext не уходит на сервер.** Приватные ключи, расшифрованные сообщения и вложения обрабатывает клиент.
2. **Доставка переживает сбои.** Транзакция БД, outbox, Kafka и восстановление по cursor.
3. **Токен могут украсть.** Refresh одноразовый, reuse token family ловится, устройство регистрируется отдельным короткоживущим token.
4. **Систему можно эксплуатировать.** Compose, Kubernetes, метрики, логи, alerts, runbooks и CI/CD.

## Что умеет сейчас

Рабочий продукт, не демо-заглушка:

- личные и групповые чаты: ответы, правки, удаление, реакции, receipts, TTL;
- голосовые: тап по микрофону переключает голос/кружок, удержание пишет, вверх — lock, влево — отмена;
- видео-кружки: запись на весь экран, у получателя это настоящее видео, не пустой кружок;
- фото и файлы: отдельное окно отправки (не полоска над инпутом), в чате листание;
- 1:1 звонки в local/dev: аудио и видео, overlay, PiP своей камеры.

Честно про ограничения:

- кружки и медиа уходят как encrypted attachment (`attachmentId` + ключ файла), не как `blob:` в JSON;
- после pull миграций **перезапусти backend** — без Flyway `V42` загрузка файлов отвечает 409;
- расшифровка голоса — Web Speech API (Chrome ок, Safari часто пусто);
- Word/Excel в чате не открываются, только скачивание;
- звонки в production выключены, пока нет TURN. Медиа всегда DTLS-SRTP; дополнительный insertable-stream E2EE есть только в Chrome. SDP сервер видит.
...
`docker-compose.dev.yml` поднимает PostgreSQL, Redis, Redpanda и coturn. Двум браузерам на одной машине coturn обычно нужен для ICE. Signaling 1:1 звонков включён в профиле `dev` (`chaos.calls.enabled=true`) и выключен в production, пока не задано `CHAOS_CALLS_ENABLED=true`.

После pull миграций **перезапусти backend**, чтобы Flyway их применил. Загрузка вложений требует `V42` (default для `object_key`); без неё `/api/attachments/upload` отвечает HTTP 409, а второй пользователь видит пустые кружки и фото.
...
Vite по умолчанию на `http://localhost:5173`.

### Два пользователя локально

1. Подними backend и frontend как выше.
2. Открой два окна (лучше два разных браузера или одно обычное + одно инкогнито).
3. Зарегистрируй два аккаунта, напиши в личку.
4. Проверь: текст, фото, голосовое, кружок — у второго должно появиться содержимое, не пустой пузырь.
5. Звонок: оба клиента на том же компьютере, coturn из `docker-compose.dev.yml` должен быть up.

Если кружок у второго пустой — это почти всегда старый backend без `V42` или старое сообщение, которое ушло с `blob:` в payload. Перезапусти API и отправь заново.
